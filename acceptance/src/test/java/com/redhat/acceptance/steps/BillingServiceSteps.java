...
//    updateProperty("com.redhat.services.billing", "billing.in",  "file://"+testFolder.getCanonicalPath()+"?fileName=in.txt");
//    updateProperty("com.redhat.services.billing", "billing.out", "file://"+testFolder.getCanonicalPath()+"?fileName=out.txt");
  ...
  public void a_billing_file_arrives_with_the_following_call_records(List<Map<String,String>> table) throws Throwable {
    ...
    sb.append("</call-details>").append("\n");
    createFile(new File(testFolder, "in.txt"), sb.toString());
  }
  
  @When("^the billing file is processed$")
  public void the_billing_file_is_processed() throws Throwable {
    File file=new File(testFolder, "out.txt");
  ...
  public void the_billing_file_call_records_should_match(List<Map<String,String>> table) throws Exception{
    File file=new File(testFolder, "out.txt");
    String content=cachedContent.get(file.getPath());
    ...
    Assert.assertEquals("Number of expected rows ["+table.size()+"] differ from actual ["+calls.size()+"]", calls.size(), table.size());
    ...
    for(Map<String,String> row:table){
      ...
      if (!calls.remove(expectedCall))
        Assert.fail("Unable to find: "+expectedCall.toString());
    }
  }
  
  public String comparableCall(String duration, String from, String fromCountry, String to, String toCountry, String type) {
